...
'''
# Run below and #2 synthetic categorical feature below to switch to another encoder
url = "https://raw.githubusercontent.com/arguz95/Project_ML2/master/Data/bankrupcy.csv"
download = requests.get(url).content
df = pd.read_csv(io.StringIO(download.decode('utf-8')))
bankruptcy = df.copy() # we can use this as raw data afterwards
...
# Creating Synthetic Categorical features & Encoding
...
encoder= ce.BaseNEncoder(cols=['Per Share Net profit before tax bin','Retained Earnings to Total Assets bin'],return_df=True,base=5)
#Fit and Transform Data
X=encoder.fit_transform(X)
X.head()
